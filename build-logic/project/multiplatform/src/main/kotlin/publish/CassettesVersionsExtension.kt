...
import at.released.cassettes.gradle.multiplatform.publish.PropertiesValueSource.Parameters
...
internal fun Project.createCassettesVersionsExtension(): CassettesVersionsExtension {
    val configFilePath: RegularFile = layout.settingsDirectory.file(VERSION_PROPERTIES_PATH)
    return extensions.create<CassettesVersionsExtension>("cassettesVersions", configFilePath)
}

open class CassettesVersionsExtension @Inject constructor(
    ...
    val rootVersion: Provider<String> = providers.gradleProperty("version")
        .orElse(providers.environmentVariable("CASSETTES_VERSION"))
        .orElse(
            propertiesProvider.map { props ->
                props["cassettes_version"]
                    ?: error("No `cassettes_version` in $propertiesFile")
            },
        )
